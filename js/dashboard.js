(async function () {
  ...
  function renderTraffic(t) {
    ...
    document.getElementById('traffic-meta').textContent =
      `Data as of: ${t.updatedAt} · ${t.reductionNote || ''} · Avoided entries estimated at avg. 73K fewer daily trips (MTA via NYT)`;
    document.getElementById('traffic-total-avoided').innerHTML =
      `<div class="big-num">~${fmt(t.estimatedTotalAvoided)}</div>
       <div class="label">Est. Vehicle Entries Avoided Since Jan 9, 2025</div>`;
